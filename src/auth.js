...
if (signInForm) {
  signInForm.addEventListener('submit', (e) => handleSignIn(e)) // Use existing event handling
}
if (signOutButton) {
...
async function updateProfilePicture() {
  ...
  try {
    ...
    // Update the profile email and UUID based on the current user
    const currentUser = auth.currentUser // Reference to the currently authenticated user
    if (currentUser) {
      profileEmail.value = currentUser.email // Set email
      profileUUID.value = currentUser.uid // Set UUID
    }

    // Update the profile image and hidden input with the new URL
    profileImage.src = url
    profilePicUrlInput.value = url // Update hidden input

    // Update Firestore with the new URL
    const userId = currentUser.uid // Use the actual user ID from the auth object
    await setDoc(doc(firestore, 'users', userId), { profilePicUrl: url })
...
// Handle sign-in
//============================/////============================///
function handleSignIn(e, email = null, password = null) {
  if (e) {
    e.preventDefault()
    e.stopPropagation()
...
if (signInForm) {
  addEventListenerWithTracking(signInForm, 'submit', (e) => handleSignIn(e)) // Handle sign-in
}
if (signOutButton) {
